...
use crate::canvas::geometries::{get_draw_mode_and_status, get_widget_id, 
    set_widget_mode_or_status_or_id, IpgCanvasWidget};
use crate::canvas::import_export::{convert_to_export, import_widgets, save};
use crate::IpgState;

use super::helpers::{get_horizontal_alignment, get_vertical_alignment, try_extract_f64, 
    try_extract_ipg_horizontal_alignment, try_extract_ipg_vertical_alignment, try_extract_point, 
            ...
            match widget {
                ...
                _ => {
                    ...
                    if draw_mode == IpgDrawMode::New {
                        app_state.last_id +=1;
                        widget = set_widget_mode_or_status_or_id(widget, 
                                                                Some(IpgDrawMode::Display), 
                                                                Some       (IpgDrawStatus::Completed), 
                                                                Some(app_state.last_id)
                                                                );
