...
ipg.add_window("main", "Menu", 600, 600,  pos_x=100, pos_y=25)
...
ipg.add_button(parent_id="row_btn", label="I'm Default Style")

ipg.add_button(parent_id="row_btn", label="I'm Custom Style",
...
ipg.add_row(window_id="main", container_id="row_many", parent_id="col",
            align_items=IpgRowAlignment.Center)
...
ipg.add_styling_border(style_id="border", radius=[12.0])

for i, label in enumerate(style_text):
    style_id = f"cont{i}"
    ipg.add_styling_background(style_id=style_id, color=background[i])
    ipg.add_button(parent_id="row_many",
                    label=label,
                    style_background=style_id,
                    style_border="border")
