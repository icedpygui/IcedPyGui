...
def widget_button(tbl_id: int, btn_id: int, index: tuple[int, int]):
    print(tbl_id, btn_id, index)


def widget_checkbox(tbl_id: int, chk_id: int, index: tuple[int, int], is_checked: bool):
    print(tbl_id, chk_id, index, is_checked)


def widget_toggler(tbl_id, tog_id: int, index: tuple[int, int], on_toggle: bool):
    print(tbl_id, tog_id, index, on_toggle)


def widget_selectable(tbl_id, sel_id, index: tuple[int, int], on_select: bool):
    print(tbl_id, sel_id, index, on_select)
...
ipg.add_table("col", "My Table", data, 
              ...
              widgets_using_columns= {0: buttons, 1: checkboxes, 2: togglers, 2: selectables},
