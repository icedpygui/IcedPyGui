...
# Add show hide text
s_h_text_id = ipg.add_text(
                    parent_id="col", 
                    content="Window 2 is closed")

# Add event text
event_text_id = ipg.add_text(
                        parent_id="col", 
                        content="This will change when an event occurs")

event_resize_id = ipg.add_text(
                        parent_id="col", 
                        content="This will change when resized event occurs")

event_close_request_id = ipg.add_text(
                                parent_id="col", 
                                content="You have no close requests")

event_focused_id = ipg.add_text(
                            parent_id="col", 
                            content="No window has the focus")

event_unfocused_id = ipg.add_text(
                            parent_id="col", 
                            content="No window has the focus")

event_file_hovered_id = ipg.add_text(
                                parent_id="col", 
                                content="No file has been hovered")

event_file_dropped_id = ipg.add_text(
                                parent_id="col", 
                                content="No file has been dropped")

event_file_hover_left_id = ipg.add_text(
                                    parent_id="col", 
